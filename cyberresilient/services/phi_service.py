...
def load_assets() -> list[dict]:
    ...
    try:
        ...
        q = tenant_filter(session.query(AssetRow), AssetRow)
        rows = q.order_by(AssetRow.name).all()
        result = []
        for r in rows:
            d = r.to_dict()
...
def get_overdue_breach_notifications() -> list[dict]:
    ...
    try:
        from cyberresilient.services.rls import tenant_filter
        rows = tenant_filter(session.query(BreachNotificationRow), BreachNotificationRow).filter_by(status="Open").all()
        overdue = []
        for r in rows:
            d = r.to_dict()
