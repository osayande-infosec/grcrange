...
def grant_ato(
    ...
    session = get_session()
    try:
        row = session.query(ATOSystemRow).filter_by(id=system_id).first()
        if not row:
...
def load_ato_systems() -> list[dict]:
    ...
    session = get_session()
    try:
        return [r.to_dict() for r in session.query(ATOSystemRow).all()]
    finally:
        session.close()
...
def create_poam(
    ...
    if _db_available():
        ...
        try:
            ...
            system = session.query(ATOSystemRow).filter_by(id=system_id).first()
            if system:
