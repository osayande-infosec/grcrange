...
st.markdown("# 🏛️ Government Compliance — NIST 800-53, FedRAMP & ATO")
st.markdown("NIST SP 800-53 Rev 5 compliance, FedRAMP ATO lifecycle, and POA&M tracking.")
st.markdown("---")
...
m1.metric("Registered Systems", len(systems))
m2.metric("Active ATOs", sum(1 for s in systems if s["status"] == "Active ATO"))
m3.metric("Open POA&Ms", psum["open"])
m4.metric("Overdue POA&Ms", psum["overdue"])
            ...
            with st.expander(
                ...
                if has_permission("admin") and s["status"] != "Active ATO":
                    if st.button(f"Grant ATO — {s['name']}", key=f"ato_{s['id']}"):
                        grant_ato(s["id"], granted_by=get_current_user().username)
                        st.success("ATO granted!")
                        st.rerun()
    ...
    if not has_permission("admin"):
        ...
        if submitted:
            if not sys_name or not owner or not isso or not ao:
                ...
                st.success(f"System '{sys_name}' registered for {impact}-baseline ATO tracking.")
                st.rerun()
